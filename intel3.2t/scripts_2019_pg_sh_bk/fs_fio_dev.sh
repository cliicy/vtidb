...
if [ "${output_dir}" == "" ];
then
        export output_dir=${result_dir}/${logfolder}-`date +%Y%m%d_%H%M%S`${case_id}_${jbs}jbs_${cpumdl}
fi
...
#time taskset 0x1 fio -filename=${mnt_point_data_pg}/${fioname}_${bsz}k.fio -iodepth 1 -thread -rw=randwrite -size=120G -randrepeat=0 -time_based -ioengine=libaio -bs=${bsz}k -numjobs=1 -runtime=60 -group_reporting -name=${fioname} > ${outfile} 
time taskset -c ${cpucore_list} fio -filename=${mnt_point_data_pg}/${fioname}_${bsz}k.fio -direct=1 -iodepth 1 -thread -rw=randwrite -size=120G -randrepeat=0 -time_based -ioengine=libaio -bs=${bsz}k -numjobs=${jbs} -runtime=${fiot} -group_reporting -name=${fioname} > ${outfile} 
echo -e "\n" >> ${outfile}
