...
if [ "${output_dir}" == "" ];
then
        export output_dir=${result_dir}/${logfolder}-`date +%Y%m%d_%H%M%S`${case_id}_${jbs}jbs_${cpumdl}_raw
fi
...
sudo time taskset -c ${cpucore_list} fio -filename=${disk} -direct=1 -iodepth 1 -thread -rw=randwrite -randrepeat=0 -time_based -ioengine=libaio -bs=${bsz}k -numjobs=${jbs} -runtime=${fiot} -group_reporting -name=${fioname} > ${outfile}
sleep 5
